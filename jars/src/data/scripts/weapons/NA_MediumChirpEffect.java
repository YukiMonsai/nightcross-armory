...
public class NA_MediumChirpEffect implements BeamEffectPlugin {

    public static float RANGE_FACTOR = 300f;
    public static float MIN_DIST = 100f;
    public static float ANGLE_MAX = 22f; // In degree


    public final int SHOCK_COUNT = 5;
    public final float DAMAGE_AMOUNT = 100;

    private static class BeamEffect {
        private IntervalUtil shockInterval = new IntervalUtil(0.075f, 0.1f);
        private int shocksLeft = 5;
        ...
        if ((target instanceof ShipAPI || target instanceof MissileAPI || target instanceof CombatAsteroidAPI) && beam.getBrightness() >= 1f
            ...
            if (data.shocksLeft > 0) {
                if (data.shockInterval.intervalElapsed()) {
                    ...
                    Global.getSoundPlayer().playSound("tachyon_lance_emp_impact", 1f, 0.5f, beam.getDamageTarget().getLocation(), Misc.ZERO);

                    engine.applyDamage(
                            beam.getDamageTarget(),
                            point, dam, DamageType.ENERGY, dam, false, false, beam.getSource(), false
                    );
                }
            }
