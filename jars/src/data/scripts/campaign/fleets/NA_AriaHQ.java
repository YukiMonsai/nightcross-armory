...
public class NA_AriaHQ extends BaseIndustry implements RouteFleetSpawner, FleetEventListener {
    ...
    public void apply() {
        ...
        market.getStats().getDynamic().getMod(Stats.GROUND_DEFENSES_MOD).modifyMult(getModId(), 1f + mult, getNameForModifier() + extra);
    ...
    protected void addPostDemandSection(TooltipMakerAPI tooltip, boolean hasDemand, IndustryTooltipMode mode) {
        if (mode != IndustryTooltipMode.NORMAL || isFunctional()) {
            ...
            addGroundDefensesImpactSection(tooltip, 1f, Commodities.SUPPLIES);
        }
    }
    ...
    public void advance(float amount) {
        ...
        if (tracker.intervalElapsed()) {
            ...
            float patrolDays = 30f + (float) Math.random() * 15f;
    ...
    public int getMaxPatrols(PatrolType type) {
        if (type == PatrolType.FAST) {
            return 2 + (int) market.getStats().getDynamic().getMod(Stats.PATROL_NUM_LIGHT_MOD).computeEffective(0);
        }
        if (type == PatrolType.COMBAT) {
            return 1 + (int) market.getStats().getDynamic().getMod(Stats.PATROL_NUM_MEDIUM_MOD).computeEffective(0);
        }
        return 0;
    }
    ...
    public CampaignFleetAPI spawnFleet(RouteData route) {
        ...
        switch (type) {
            ...
            case COMBAT:
                ...
                break;
            case HEAVY:
                combat = Math.round(28 + (float) random.nextFloat() * 3f) * 5f;
