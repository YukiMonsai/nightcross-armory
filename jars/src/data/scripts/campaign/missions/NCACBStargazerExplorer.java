...
public class NCACBStargazerExplorer extends BaseCustomBountyCreator {
...
//		smallUsePhase = true;
        ...
        if (smallFleet) {
            data.custom1 = true;
            if (difficulty <= 6) {
                mission.triggerCustomAction(new CreateFleetAction(FleetTypes.SCAVENGER_SMALL, data.system.getLocation(), FleetSize.SMALL, FleetQuality.SMOD_3,
                        Factions.SCAVENGERS));
                mission.triggerSetFleetMaxShipSize(1);
                mission.triggerSetFleetDoctrineOther(1, 4);
            } else if (difficulty <= 7) {
                mission.triggerCustomAction(new CreateFleetAction(FleetTypes.SCAVENGER_SMALL, data.system.getLocation(), FleetSize.MEDIUM, FleetQuality.SMOD_3,
                        Factions.SCAVENGERS));
                ...
                mission.triggerCustomAction(new CreateFleetAction(FleetTypes.SCAVENGER_SMALL, data.system.getLocation(), FleetSize.LARGE, FleetQuality.SMOD_3,
                        Factions.SCAVENGERS));
            ...
            if (difficulty <= 6) {
                mission.triggerCustomAction(new CreateFleetAction(FleetTypes.SCAVENGER_SMALL, data.system.getLocation(), FleetSize.MEDIUM, FleetQuality.VERY_HIGH,
                        Factions.SCAVENGERS));
            } else if (difficulty <= 7) {
                mission.triggerCustomAction(new CreateFleetAction(FleetTypes.SCAVENGER_MEDIUM, data.system.getLocation(), FleetSize.LARGE, FleetQuality.VERY_HIGH,
                        Factions.SCAVENGERS));
            } else if (difficulty <= 8) {
                mission.triggerCustomAction(new CreateFleetAction(FleetTypes.SCAVENGER_LARGE, data.system.getLocation(), FleetSize.VERY_LARGE, FleetQuality.VERY_HIGH,
                        Factions.SCAVENGERS));
            } else if (difficulty <= 9) {
                mission.triggerCustomAction(new CreateFleetAction(FleetTypes.SCAVENGER_LARGE, data.system.getLocation(), FleetSize.VERY_LARGE, FleetQuality.SMOD_1,
                        Factions.SCAVENGERS));
            } else if (difficulty <= 10) {
                mission.triggerCustomAction(new CreateFleetAction(FleetTypes.SCAVENGER_LARGE, data.system.getLocation(), FleetSize.HUGE, FleetQuality.SMOD_2,
                        Factions.SCAVENGERS));
            }
        ...
        mission.setTimeLimit(BaseCustomBounty.Stage.FAILED, 90f, data.system);
        ...
        data.baseReward = CBStats.getBaseBounty(difficulty, CBStats.PIRATE_MULT - 0.25f, mission);
    ...
    public void updateInteractionData(HubMissionWithBarEvent mission, CustomBountyData data) {
        ...
        if (data.custom1 != null) {
            mission.set("$" + id + "_ncastargazerexplorer", data.difficulty);
            mission.set("$bcb_ncastargazerexplorer", data.difficulty);
        } else {
            mission.unset("$" + id + "_ncastargazerexplorer");
            mission.unset("$bcb_ncastargazerexplorer");
        }
    }
...
//			}
            ...
            params.addShips.add("apogee"); // the quintissential explorer ship
            params.addShips.add("na_nammu"); // the not-so-quintissential

            context.fleet = FleetFactoryV3.createFleet(params);
            context.fleet.setFacing(random.nextFloat() * 360f);

            // goodies
            context.fleet.addDropRandom("na_weps_stargazer", Math.round(5 + random.nextFloat() * fSize.maxFPFraction * 5));
            context.fleet.addDropRandom("na_weps_stargazer_loot", Math.round(5 + random.nextFloat() * fSize.maxFPFraction * 5));
